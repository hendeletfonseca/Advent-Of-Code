...
while len(problem_input) > 0:
    i = problem_input.find("mul(")
    if i == -1:
        break
    problem_input = problem_input[i+4:]
    
    x_size = 3
    x=-1

    if len(problem_input) < x_size:
        break
    while (x_size > 0 and x == -1):    
        temp_x = problem_input[:x_size]
        try:
            x = int(temp_x)
        except ValueError:
            x_size -= 1

    problem_input = problem_input[x_size:]
    
    if problem_input[0] != ",":
        continue
    problem_input = problem_input[1:]

    y_size = 3
    y=-1

    if len(problem_input) < 2: # 2 because we need at least 1 digit and 1 closing parenthesis
        break

    if len(problem_input) < y_size:
        y_size = len(problem_input) - 1

    while (y_size > 0 and y == -1):    
        temp_y = problem_input[:y_size]
        try:
            y = int(temp_y)
        except ValueError:
            y_size -= 1

    problem_input = problem_input[y_size:]
...
while len(problem_input) > 0:
    i = problem_input.find("mul(")
    j = problem_input.find("do")
    if j!=-1 and j < i:
        if problem_input.find("don't") == j:
            do = False
            problem_input = problem_input[j+1:]        
        else:
            do = True    
            problem_input = problem_input[j+1:]
        continue
